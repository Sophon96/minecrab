...
pub fn tick(gd: &mut GameData, rl: &mut RaylibHandle) {
    ...
    if gd.paused {
        ...
        if rl.is_key_pressed(KeyboardKey::KEY_Q) {
            let buf = rmp_serde::to_vec(&gd.world).expect("serialize failed");
        ...
        if rl.is_key_pressed(KeyboardKey::KEY_L) {
            ...
            gd.world = rmp_serde::from_slice(&bytes).expect("deserialize failed");

            // reset world renderer
            gd.world.mesh_all_chunks(&mut gd.world_renderer);
        }
        ...
        if rl.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) {
            ...
            if let Some(h) = hit {
                (&mut gd.world).set_block_data(h.x, h.y, h.z, BlockData::AIR);
        ...
        let Vector3 { x: px, y: py, z: pz } = (&mut gd.player).camera.position;
        (&mut gd.world).generate_surrounding_chunks(px as i64, py as i64, pz as i64, 1);
        (&mut gd.world).poll_chunk_gen_thread(&mut gd.world_renderer);
